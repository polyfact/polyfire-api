...
func main() {
	...
	// Auth Routes
	router.GET(
		"/project/:id/auth/firebase",
		middlewares.Record(utils.AuthFirebase, auth.ExternalFirebaseTokenExchangeHandler),
	)
	router.GET(
		"/project/:id/auth/custom",
		middlewares.Record(utils.AuthCustom, auth.ExternalCustomTokenExchangeHandler),
	)
	router.GET(
		"/project/:id/auth/anonymous",
		middlewares.Record(utils.AuthAnonymous, auth.AnonymousTokenExchangeHandler),
	)
	router.GET(
		"/project/:id/auth/provider/redirect",
		middlewares.Record(utils.AuthProviderRedirection, auth.RedirectAuth),
	)
	router.GET(
		"/project/:id/auth/provider/callback",
		middlewares.Record(utils.AuthProviderCallback, auth.CallbackAuth),
	)
	router.POST(
		"/project/:id/auth/provider/refresh",
		middlewares.Record(utils.AuthProviderRefresh, auth.RefreshToken),
	)
	...
	router.POST(
		"/generate",
		middlewares.Record(utils.Generate, middlewares.Auth(completion.Generate)),
	)
	router.GET(
		"/chat/:id/history",
		middlewares.Record(utils.ChatHistory, middlewares.Auth(completion.GetChatHistory)),
	)
	router.GET("/chats", middlewares.Record(utils.ChatList, middlewares.Auth(completion.ListChat)))
	router.POST(
		"/chats",
		middlewares.Record(utils.ChatCreate, middlewares.Auth(completion.CreateChat)),
	)
	router.PUT(
		"/chat/:id",
		middlewares.Record(utils.ChatUpdate, middlewares.Auth(completion.UpdateChat)),
	)
	router.DELETE(
		"/chat/:id",
		middlewares.Record(utils.ChatDelete, middlewares.Auth(completion.DeleteChat)),
	)
	router.GET(
		"/stream",
		middlewares.Record(utils.Generate, middlewares.AuthStream(completion.Stream)),
	)

	// Transcription Routes
	router.POST(
		"/transcribe",
		middlewares.Record(utils.SpeechToText, middlewares.Auth(stt.Transcribe)),
	)
	...
	router.GET(
		"/image/generate",
		middlewares.Record(
			utils.ImageGeneration,
			middlewares.Auth(imageGeneration.ImageGeneration),
		),
	)
	...
	router.POST(
		"/memory/:id/search",
		middlewares.Record(utils.MemorySearch, middlewares.Auth(memory.Search)),
	)
